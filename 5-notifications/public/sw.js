...
(function () {
    ...
    /** ======= BACKGROUND SYNC ====== */
    /**
     * this event is not fetching until there is an stable internet connection, 
     * otherwise it will be suspended.
     * 
     * How the process happens? 
     *  - First at your app js, check if sw is ready then save data to indexedDB
     *    then register to the sw a new backround sync task with a specific name.
     * 
     *  - In sw add the sync eventlistener then fetch your post request,
     *    with data brought from the indexedDB
     * 
     * 
     *  Take a look at (Periodic background sync) - (background fetch) 
     * 
     */
            ...
            event.waitUntil(async function () {
                try {
                    ...
                    data.forEach(async el => {
                        ...
                        if (response.ok) {
                            clearSingledbData('sync-posts', response.body.id);
                        }
                    })
                } catch (error) {
                    console.log(error);
